...
export function ExportPanel({ className }: ExportPanelProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const result = await PlantUMLExporter.exportWorkflowToPUML(workflow, options);
  ...
  const handlePreview = () => {
    try {
      ...
      const pumlContent = PlantUMLExporter.convertWorkflowToPlantUML(workflow, options);
      
      // Show preview in a modal or new window
      const previewWindow = window.open('', 'PlantUML Preview', 'width=800,height=600');
      if (previewWindow) {
      ...
      alert(`Preview error: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  };
  ...
  return (
    <Card className={cn('p-4 space-y-4', className)}>
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-lg font-semibold text-slate-900">PlantUML Export</h3>
      </div>
      ...
      <div className="space-y-3">
        <div>
          ...
          <div className="mt-2 space-y-2">
            <label className="flex items-center gap-2">
              ...
              />
              <span className="text-sm text-slate-600">Include node properties</span>
            </label>
        ...
        <div>
          ...
          <div className="mt-1 text-sm text-slate-600 bg-slate-50 p-2 rounded border">
            {options.outputPath || PlantUMLExporter.getDefaultExportPath()}*.puml
          </div>
