...
export function useWorkflowTabs(): UseWorkflowTabsReturn {
  ...
  const loadUMLFiles = useCallback(async (): Promise<UMLFileInfo[]> => {
    try {
      ...
      for (const file of files) {
        try {
          const content =
            localStorage.getItem(`puml-content-${file.path}`) || '';
  ...
  const saveCurrentCanvasToTab = useCallback(
    (tabId: string) => {
      ...
      try {
        ...
        if (result.success && result.filePath) {
          ...
          setState(prev => ({
            ...prev,
            tabs: prev.tabs.map(t =>
              t.id === tabId ? { ...t, path: result.filePath, saved: true } : t
            ),
          }));
          ...
          if (result.content) {
            ...
            if (!fileExists) {
              const workflowName =
                updatedTab.workflow?.name || updatedTab.name || 'workflow';
              const newFile = {
          ...
          console.log(
            `Saved workflow "${updatedTab.name}" to ${result.filePath}`
          );
        } else {
    ...
    (tabId: string) => {
      ...
      if (tab.workflow) {
        try {
          ...
          console.log(
            `✓ Loading workflow: ${nodes.length} nodes, ${edges.length} edges`
          );
          console.log(
          ...
          if (nodes.length > 0) {
            ...
            if (plantUMLNodes.length > 0) {
              console.log(
                `🌱 PlantUML-derived nodes detected: ${plantUMLNodes.length}/${nodes.length}`
              );
              console.log(
  ...
  const renameTab = useCallback(
    async (tabId: string, newName: string) => {
      ...
      setState(prev => ({
        ...prev,
        tabs: prev.tabs.map(t =>
          t.id === tabId ? { ...t, name: sanitizedName, modified: true } : t
        ),
      }));
...
@enduml`,
    ...
    for (const sample of sampleFiles) {
      ...
      console.log(
        `  ✅ Created sample file: ${sample.name} (${sample.fileName})`
      );
    }
    ...
    console.log(
      `🎉 Successfully created ${sampleFiles.length} default sample files`
    );
  }, []);
  ...
  const loadTabsFromUML = useCallback(async () => {
    ...
    console.log(
      '  - ai-ley-puml-files:',
      localStorage.getItem('ai-ley-puml-files')
    );

    // Check if our specific PlantUML content exists
    const buildProjectContent = localStorage.getItem(
      'puml-content-../../.ai-ley/shared/uml-flows/user/build-project.puml'
    );
    console.log(
      '  - build-project.puml content length:',
      buildProjectContent?.length || 'NOT FOUND'
    );
    if (buildProjectContent) {
      console.log(
        '  - build-project.puml preview:',
        `${buildProjectContent.substring(0, 200)}...`
      );
    }
    ...
    try {
      console.log(
        '📂 Step 1: Loading UML files from ../../.ai-ley/shared/uml-flows/user/'
      );
      ...
      if (umlFiles.length === 0) {
        ...
        console.log(
          `✨ After creating samples: Found ${umlFiles.length} UML files`
        );
      }
      ...
      for (let i = 0; i < umlFiles.length; i++) {
        ...
        console.log(
          `  📄 Processing file ${i + 1}/${umlFiles.length}: ${file.name}`
        );
        ...
        try {
          ...
          if (workflow) {
            ...
            newTabs.push(tab);
            console.log(
              `    ✨ Tab created and added to newTabs array (total: ${newTabs.length})`
            );
          } else {
            console.warn(
              `    ⚠️ importFromPUML returned null/undefined for ${file.path}`
            );
          }
        } catch (fileError) {
          ...
          console.error(
            '    Error stack:',
            fileError instanceof Error ? fileError.stack : 'No stack available'
          );
        }
      }

      console.log(
        `📋 Step 4: Processed all files. Total tabs created: ${newTabs.length}`
      );

      // If no tabs were loaded, create a default tab
      if (newTabs.length === 0) {
        console.log(
          '🆕 Step 5a: No tabs loaded - creating default Welcome tab'
        );
      ...
      console.log(
        `  🎯 Setting active tab to: "${newTabs[0].name}" (ID: ${activeTabId})`
      );
      console.log('  📋 All tabs being set:');
      newTabs.forEach((tab, index) => {
        console.log(
          `    ${index + 1}. "${tab.name}" (ID: ${tab.id}, saved: ${tab.saved}, modified: ${tab.modified})`
        );
      });
  ...
  const markTabModified = useCallback((tabId: string) => {
    setState(prev => ({
      ...prev,
      tabs: prev.tabs.map(t =>
        t.id === tabId ? { ...t, modified: true, saved: false } : t
      ),
    }));
  }, []);
