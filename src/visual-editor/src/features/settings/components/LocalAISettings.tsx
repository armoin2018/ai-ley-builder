...
import { Plus, Trash2, Edit2, Terminal, Settings } from 'lucide-react';
import { Button, Input, Label } from '../../../shared/components';
import { useSettings } from '../../../hooks/useSettings';
import type { LocalAITool } from '../../../types/settings';

export function LocalAISettings() {
  const { settings, saveLocalAITool, removeLocalAITool, updateLocalAISettings } = useSettings();
  ...
  const parseArgs = (argsString: string): string[] => {
    return argsString.split(',').map(arg => arg.trim()).filter(Boolean);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 pb-3 border-b border-slate-200">
        ...
        <div>
          <h3 className="text-lg font-semibold text-slate-900">Local AI Tools</h3>
          <p className="text-sm text-slate-600">Configure local AI CLI tools and command-line interfaces</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="default-tool" className="text-sm font-medium text-slate-700">
              Default Tool
            </Label>
            <select
              id="default-tool"
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h4 className="text-sm font-semibold text-slate-700">Configured Tools</h4>
          <Button
            variant="outline"
            size="sm"
        ...
        {localAI.tools.length === 0 ? (
          <div className="text-center py-8 text-slate-500">
            ...
            <p className="text-xs">Add your first local AI tool to get started</p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h5 className="font-medium text-slate-900">{tool.name}</h5>
                      <span
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="tool-command" className="text-sm font-medium text-slate-700">
                  Command *
                </Label>
                <Input
                  id="tool-command"
              ...
              <div className="space-y-2">
                <Label htmlFor="tool-args" className="text-sm font-medium text-slate-700">
                  Arguments
                </Label>
                <Input
                  id="tool-args"
                  ...
                  onChange={(e) => updateEditingTool({ args: parseArgs(e.target.value) })}
                  placeholder="e.g., run, --model, llama2"
                  className="w-full"
                />
                <p className="text-xs text-slate-500">Comma-separated list of arguments</p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="tool-description" className="text-sm font-medium text-slate-700">
                  Description
                </Label>
                <Input
                  id="tool-description"
              ...
              <div className="flex items-center justify-between">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    onChange={(e) => updateEditingTool({ enabled: e.target.checked })}
            ...
            <div className="flex items-center gap-2 mt-6 pt-4 border-t border-slate-200">
              ...
              <Button
                ...
                disabled={!editingTool.name.trim() || !editingTool.command.trim()}
                className="flex-1"
              >
