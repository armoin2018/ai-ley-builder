...
export interface WorkflowOperations {
  ...
  // PlantUML auto-save functionality
  enablePUMLAutoSave: () => void;
  disablePUMLAutoSave: () => void;
}
...
export function useWorkflow(): UseWorkflowReturn {
  ...
  const [state, setState] = useState<WorkflowState>({
    ...
    lastSaved: null,
    error: null,
    pumlAutoSaveEnabled: true, // Enable PlantUML auto-save by default
  });
  ...
  const saveWorkflow = useCallback(
    async (name?: string, description?: string) => {
      ...
      try {
        ...
        // Auto-export to PlantUML if enabled
  ...
  // Auto-load PlantUML workflow
  const autoLoadPUML = useCallback(async () => {
    setState(prev => ({ ...prev, isLoading: true, error: null }));
    ...
    try {
      ...
      // Deserialize the PlantUML workflow
      const { nodes, edges, viewport } = deserializeWorkflow(pumlWorkflow);
  ...
  // Enable PlantUML auto-save
  const enablePUMLAutoSave = useCallback(() => {
    setState(prev => ({ ...prev, pumlAutoSaveEnabled: true }));
    console.log('PlantUML auto-save enabled');
  }, []);

  // Disable PlantUML auto-save
  const disablePUMLAutoSave = useCallback(() => {
    setState(prev => ({ ...prev, pumlAutoSaveEnabled: false }));
    console.log('PlantUML auto-save disabled');
  }, []);
