...
function getDefaultPropertiesForNodeType(nodeType: string): Record<string, any> {
  switch (nodeType) {
...
export function parsePlantUMLToFlow(plantumlContent: string): ParsedFlow {
  ...
  console.log('📄 Content preview (first 500 chars):', plantumlContent.substring(0, 500));
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (activityMatch) {
      ...
      const nodeType = currentNodeMetadata?.type || getActivityNodeType(activityName);
      nodeMap.set(alias, nodeId);
      
      console.log(`✅ Found activity node: "${activityName}" -> ${nodeType} (${nodeId})`);
    ...
    const rectMatch = line.match(/^(rectangle|component|actor|database|cloud|folder|frame|package|node)\s+"([^"]+)"\s+as\s+(\w+)/);
    if (rectMatch) {
    ...
    const simpleRectMatch = line.match(/^(rectangle|component|actor|database|cloud|folder|frame|package|node)\s+"([^"]+)"/);
    if (simpleRectMatch) {
...
function createImplicitActivityConnections(nodes: Node[], edges: Edge[], nodeMap: Map<string, string>): void {
...
function getActivityNodeType(activityName: string): string {
  ...
  if (activity.includes('build') || activity.includes('create') || activity.includes('generate')) {
    return 'command-prompt-file';
  }
  if (activity.includes('plan') || activity.includes('design') || activity.includes('architecture')) {
    return 'instruction';
  }
  if (activity.includes('innovate') || activity.includes('analyze')) {
    return 'custom-prompt';
  }
  if (activity.includes('run') || activity.includes('execute') || activity.includes('implement')) {
    return 'command-prompt-file';
  }
  if (activity.includes('test') || activity.includes('document')) {
    return 'output-formatter';
  }
  if (activity.includes('business') || activity.includes('marketing') || activity.includes('revenue')) {
    return 'custom-prompt';
  }
  if (activity.includes('launch') || activity.includes('market') || activity.includes('canvas') || activity.includes('pitch')) {
    return 'output-formatter';
  }
...
export function flowToPlantUML(nodes: Node[], edges: Edge[], workflowName: string): string {
  const lines: string[] = [];
  ...
  for (const edge of edges) {
    ...
    if (sourceNode && targetNode) {
      ...
      if (edge.data && Object.keys(edge.data).length > 1) { // More than just arrow
        lines.push(`note on link`);
        lines.push(`  **Edge ID:** ${edge.id}`);
        for (const [key, value] of Object.entries(edge.data)) {
          if (key !== 'arrow' && value !== undefined && value !== null && value !== '') {
